...
function(add_analyzer_plugin TARGET)
    ...
    # analyzer list. Use a POST_BUILD step with generator expressions so
    # this stays correct for any config/generator, on every rebuild,
    # automatically.
    #
    # It's copied into a `deps` subfolder rather than directly alongside
    # the plugin: confirmed on Windows that Logic 2 scans its configured
    # Custom Low Level Analyzers directory and tries to load every .dll
    # found there as a plugin candidate, throwing a "Failed to Load Custom
    # Analyzer ... Loaded library does not have required functions" error
    # for the SDK runtime itself (correctly, since it isn't a plugin) --
    # but only checks files directly in that directory, not subfolders.
    # Windows dependency resolution for the plugin DLL still finds it one
    # level down in `deps`, so this keeps both working: no spurious error,
    # plugin still loads.
    if (APPLE OR WIN32)
